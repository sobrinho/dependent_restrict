...
module DependentProtect
  ...
  module ClassMethods
    ...
    def add_dependency_callback!(reflection, options)
      ...
      when :rollback
        ...
        module_eval "before_destroy { |record|
            if !record.#{reflection.name}.empty?
      ...
      when :raise
        ...
        module_eval "before_destroy { |record|
            if !record.#{reflection.name}.empty?
              raise ActiveRecord::DependencyError, \"#{dependency_error(reflection)}\"
            end
            record.errors.empty?
        }"
      end
    ...
    def dependency_error(reflection)
      ...
      count_code = "record.send(reflection.name).count"
      first_five_code = 'record.send(reflection.name).first(5).map{|o| "#{o.id}: #{o.to_s}"}'
      DESTROY_PROTECT_ERROR_MESSAGE.
        ...
        gsub('dependee_type', '#{record.class.to_s.underscore.gsub(\'_\', \' \')}').
        gsub('dependee', '#{record}')
    end
  end
end
